...
The y variable (or response variable, or independent variable) goes on the left, followed by a tilde (`~`), followed by the predictor or dependent variable(s). In our example above, we want to predict extra sleep (`extra`) as a function of drug type (confusingly named `group` column). Thus our R formula should look like this:
