...
There appears to be a negative relationship between the two variables. Note that you could optionally change the colors, x- and y-axis labels, etc. in the same way that we changed things in previous plots (see section 5 of this manual).
...
To get a normal probability plot (or QQ-plot, or normal quantile plot) for the dependent variable, see section 5 of the manual ("Normal Quantile Plot).

There is a large amount of hidden information available in our regression output object (`mpg.lm`). We can extract a list of the unstandardized residuals by calling the `residuals()` function on our model output:
